...
export class Player extends GameObject implements IPlayer {
  ...
  update(touchState: ITouchState): void {
    if (touchState.move.active) {
      ...
      if (dist > 0) {
        ...
        this.vx = Math.cos(moveAngle) * PLAYER_SPEED * moveMagnitude;
        this.vy = Math.sin(moveAngle) * PLAYER_SPEED * moveMagnitude;
  ...
  // Note: shoot() logic is moved to useGameState/logic.ts to manage projectiles array
}
